...
### 🎯 프로젝트 7: 일정 자동 추출 및 Google Calendar 등록 시스템 ⭐

#### 목표

```
Gmail 이메일에서 일정을 자동 추출하여 Google Calendar 등록
한국어 날짜 표현도 정확하게 파싱 ("다음 주 화요일" → 자동 변환)
```

#### 핵심 기능

**1. 지능형 일정 추출**
- AI가 이메일 내용에서 미팅, 마감일, 행사 자동 감지
- 한국어 자연어 날짜/시간 처리
- 모호한 표현도 해석 ("다음 주 중에" → 날짜 후보 제시)

**2. 중복 방지 시스템**
- Levenshtein Distance로 유사한 일정 감지
- 같은 시간대 중복 체크
- 이미 등록된 일정 자동 건너뛰기
...
```
시나리오 1: 명확한 미팅 초대
수신: "다음 주 금요일 오후 2시에 Q4 전략 회의"
→ AI 추출: 2024-11-22 14:00, 확신도 95%
→ 자동 등록 완료 ✅

시나리오 2: 프로젝트 마감일
수신: "기획서는 11월 29일 금요일까지 제출"
→ AI 추출: 마감일, 2024-11-29 18:00 (업무 종료 시간 추정)
→ 자동 등록 + 1일 전 알림 설정 ✅
...
- **시간 절감**: 일정 1개당 10분 → 0분 (하루 5개 = 50분 절약)
- **정확도**: 수동 입력 실수율 5-10% → AI 정확도 92-98%
- **월간 ROI**: 20시간 절약 = 약 62만원 (개인), 600만원 (팀 10명)

**상세 가이드**: [`05-use-cases/schedule-intelligence/`](05-use-cases/schedule-intelligence/)
- [OVERVIEW.md](05-use-cases/schedule-intelligence/OVERVIEW.md) - 프로젝트 소개 및 시나리오
- [README.md](05-use-cases/schedule-intelligence/README.md) - 상세 구현 가이드
